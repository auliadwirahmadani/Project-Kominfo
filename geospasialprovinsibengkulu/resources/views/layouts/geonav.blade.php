...
<nav class="fixed top-4 left-1/2 -translate-x-1/2 w-[95vw] max-w-[1440px] h-16
            ...
            <div class="relative" x-data="{ filterOpen: false }">
                ...
                <div x-show="filterOpen" 
                    ...
                    <div class="px-4 pt-2 border-t border-gray-100 flex gap-2">
                        <button onclick="resetFilterPeta()" 
                                class="flex-1 px-3 py-2 text-xs font-medium text-gray-700 bg-gray-100 
                                       rounded-lg hover:bg-gray-200 transition">
                            Reset
                        </button>
                        <button id="btnTerapkan" onclick="terapkanFilterPeta()"
    ...
    <div id="mobileSearchPanel" 
        ...
        <div class="flex gap-2">
            ...
            <button onclick="terapkanFilterMobile()" class="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition whitespace-nowrap">
                Filter
            </button>
        </div>
    </div>
...
<script>
// ==========================================
// 1. FUNGSI FILTER UTAMA (DIJAMIN JALAN)
// ==========================================
window.terapkanFilterPeta = function() {
    ...
    fetch(`/geospatial/filter?category_id=${categoryId}&year=${year}`)
        .then(res => {
            if (!res.ok) throw new Error("Gagal mengambil data ke Database");
            return res.json();
        })
        .then(data => {
            ...
            if (dataPeta.length === 0) {
                alert("Pemberitahuan: Tidak ada data peta untuk kategori/tahun ini.");
            }

            if (typeof window.loadMapData === 'function') {
                window.loadMapData(dataPeta);
            } else {
                alert("Error: Fungsi pembuat peta (Leaflet) gagal dimuat.");
            }
        })
        .catch(err => {
...
// ==========================================
// 2. ALPINE.JS KOMPONEN DROPDOWN SEARCH
// ==========================================
document.addEventListener('alpine:init', () => {
    Alpine.data('layerSearch', () => ({
        open: false, query: '', layers: [], filteredLayers: [], selected: null,
        ...
        search() {
            if (this.query.trim() === '') {
                ...
                this.filteredLayers = this.layers.filter(l => l.name.toLowerCase().includes(q));
            }
        },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(mobileSearchToggle && mobileSearchPanel) {
        mobileSearchToggle.addEventListener('click', function(e) {
            e.stopPropagation();
            mobileSearchPanel.style.display = mobileSearchPanel.style.display === 'block' ? 'none' : 'block';
        });
    }
});
</script>
